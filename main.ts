...
import { pruneOrphans } from "./src/repo/attachments.ts";
...
if (isDesktop && await focusRunningInstance()) Deno.exit(0);

getDb();
// Verwaiste Anhangdateien (etwa nach dem Zurückspielen einer Sicherung)
// beiläufig aufräumen — nicht blockierend, der Start soll darauf nicht warten.
void pruneOrphans().catch((error) => log.warn(`Aufräumen der Anhänge übersprungen: ${error}`));
